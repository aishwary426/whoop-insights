        ...
        >
            ...
            <div className="grid md:grid-cols-3 gap-6">
                ...
                <div className="md:col-span-2 glass-card p-6 bg-gradient-to-br from-neon-primary/5 to-transparent border-neon-primary/20">
                    <div className="flex items-start justify-between mb-4">
                        <div>
                            ...
                            <h3 className="text-3xl font-bold text-white">{analysis.winner.sport_type}</h3>
                        </div>
                        <div className="text-right">
                            <div className="text-3xl font-bold text-neon-primary">{analysis.winner.avg_cal_per_min.toFixed(1)}</div>
                            <div className="text-xs text-white/50">cal/min</div>
                        </div>
                    </div>

                    <p className="text-white/80 leading-relaxed text-sm border-t border-white/10 pt-4 mt-2">
                    ...
                    <div className="grid grid-cols-2 gap-4 mt-6">
                        <div className="bg-black/20 rounded-lg p-3">
                            ...
                            <div className="text-lg font-semibold">{analysis.winner.avg_hr.toFixed(0)} <span className="text-xs font-normal text-white/40">bpm</span></div>
                        </div>
                        <div className="bg-black/20 rounded-lg p-3">
                            <div className="flex items-center gap-2 text-xs text-white/50 mb-1">
                                <TrendingUp className="w-3 h-3" /> Samples
                            </div>
                            <div className="text-lg font-semibold">{analysis.winner.sample_size} <span className="text-xs font-normal text-white/40">sessions</span></div>
                        </div>
                    </div>
                </div>
                ...
                <div className="glass-card p-6">
                    <h4 className="text-sm font-semibold text-white/60 mb-4 uppercase tracking-wider">Efficiency Ranking</h4>
                        ...
                        {analysis.comparison.slice(0, 4).map((item, idx) => (
                            <div key={item.sport_type} className="flex items-center justify-between group">
                                <div className="flex items-center gap-3">
                                    <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${idx === 0 ? 'bg-neon-primary text-black' : 'bg-white/10 text-white/60'}`}>
                                        {idx + 1}
                                    </div>
                                    <span className={idx === 0 ? 'text-white font-medium' : 'text-white/60'}>{item.sport_type}</span>
                                </div>
                                <div className="text-sm font-mono">
                                    <span className={idx === 0 ? 'text-neon-primary' : 'text-white/40'}>{item.avg_cal_per_min.toFixed(1)}</span>
                                </div>
                            </div>
                        ))}
                    </div>
