...
export const StatCard = memo(function StatCard({ icon: Icon, label, value, subtitle, color, delay = 0 }: StatCardProps) {
  return (
    <NeonCard
      className="p-3 md:p-6 border-gray-200 dark:border-white/10 group hover:border-neon-primary/30 transition-colors"
    >
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="flex flex-col items-end flex-shrink-0">
          ...
          {subtitle && <div className="text-[9px] md:text-[10px] uppercase tracking-wider text-gray-400 dark:text-white/30 mt-0.5 md:mt-1">{subtitle}</div>}
        </div>
      </div>
    </NeonCard>
  )
