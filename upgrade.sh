...
export default function SignupPage() {
  ...
  return (
    <div className="min-h-screen relative overflow-hidden flex items-center justify-center px-6 py-12">
      {/* Same background and particles as login */}
      ...
      <div className="relative z-10 max-w-md w-full">
        <div className="text-center mb-8 animate-float">
          <Link href="/" className="inline-block">
            ...
            <h1 className="text-4xl font-bold gradient-text mb-2">Whoop Insights Pro</h1>
        ...
        <div className="glass-white rounded-3xl p-8 shadow-2xl glow">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              <div className="relative">
                <input type={showPassword ? "text" : "password"} name="password" value={formData.password} onChange={handleChange}
                  className="w-full glass px-6 py-4 rounded-2xl text-gray-900 placeholder-gray-400 focus:ring-2 focus:ring-purple-500/50 outline-none transition-all duration-300 bg-white/50"
                  placeholder="••••••••" required style={!showPassword ? { letterSpacing: '0.3em' } : {}} />
            ...
            <button type="submit" disabled={loading} className="btn-primary w-full relative overflow-hidden group">
              <span className="relative z-10">
                {loading ? <span className="flex items-center justify-center gap-2">
                  <div className="spinner !w-5 !h-5 !border-2"></div>Creating account...
                </span> : 'Create Account →'}
              </span>
            </button>
          </form>
          ...
          <button onClick={handleGoogleSignUp} type="button"
            className="w-full glass flex items-center justify-center gap-3 py-4 rounded-2xl hover:bg-white/30 transition-all duration-300 font-semibold text-gray-700 group">
